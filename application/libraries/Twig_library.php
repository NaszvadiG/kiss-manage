...
class Twig_library {
	...
	public function __construct() 
	{
		...
		require_once (string)'ExtensionInterface.php';		
		require_once (string)'Extension.php';
		require_once (string)APPPATH . 'third_party/Twig/extensions/applyfilterextension.php';
        require_once (string)APPPATH . 'third_party/Twig/extensions/Text.php';
		// register autoloader
	 ...
	 * 
	 */
	public function render($data = array(), $render = TRUE, $template = false) 
	{	
		if($template === false) {
	 ...
	 * 
	 */
	public function display($data = array(), $template = false) 
	{
		if($template === false) {
			$template = $this->getTemplateName();
		}
	...
	/**
	 * Get the name of the template based on controller and method
	 * 
	 * 
	 * @return string
	 * 
	...
	public function getTemplateName() {
		return $this->_ci->router->class . '/' . $this->_ci->router->method .'.tpl';
	}
	...
	public function ci_function_init() 
	{
        ...
        $this->_twig_env->addExtension(new ApplyFilterExtension());
